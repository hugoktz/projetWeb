...
<body>
    ...
    <?php
        ...
        if(isset($_POST['save']))
        {
            ...
            VALUES (?, ?, ?, ?, ?, ?);
            INSERT INTO being_located_in (id_place, id_company)
            SELECT places.id_place, company.id_company
            FROM places, company
            WHERE places.street_name = ? and company_name = ?;");

            $requete->execute(array($city, $street_name, $street_number, $postal_code, $company_name, $company_sector, $nb_CESI_intern, $interns_evaluation, $Pilote_trust, $internship_nb_available, $street_name, $company_name));
